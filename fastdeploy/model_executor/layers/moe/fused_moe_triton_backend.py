    ...
    def process_weights_after_loading(self, layer):
        ...
        def _process_quantize(weight_idx):
            ...
            for expert_id in range(layer.num_local_experts):
                weight_quant, weight_scale_fp32 = quant_weight_ue8m0(
                    weight_dequant=getattr(layer, unquantized_weight_name)[expert_id].transpose([1, 0]).contiguous(),
                    weight_block_size=self.quant_config.weight_block_size,
                )
                weight_scale = transform_scale_ue8m0(
                    weight_scale_fp32, mn=weight_quant.shape[-2], weight_block_size=self.quant_config.weight_block_size
                )
                weight[expert_id].copy_(weight_quant, False)
